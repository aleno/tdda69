...
(define (eval-definition exp env)
  (define-variable! (if (variable? (definition-variable exp))
    ...
    (if (null? varlist)
        ...
        (begin
          (eval-sequence (dolist-actions exp)
                         (extend-environment ;; Sätt x till första elementet i varlist
                          (car (dolist-args exp))
                          (car varlist)
                          env))
          (loop (cdr varlist)))))
...
(remote-eval '(%defmacro %dolistm
  (%lambda args 
    (%list (%quote %begin)
       (%list (%quote %define) (%list (%quote loop) (%quote li))
         (%list (%quote %if) (%list (%quote %null?) (%quote li))
             (%list (%quote %list))
             (%list (%quote %let) (%list (%list (%car (%car args)) (%list (%quote %car) (%quote li))))
               (%cons (%quote %begin) (%cdr args))
               (%list (%quote loop) (%list (%quote %cdr) (%quote li))))))
       (%list (%quote loop) (%cadr (%car args)))
       (%quote sum)))))

(display "test dolistm")
(remote-eval '(%define (%sumlist2 1st)
                       (%define sum 0)
                       (%dolistm (x 1st sum) (%set! sum (%+ sum x)))))
(remote-eval '(%sumlist2 (%quote (1 2 3))))

(remote-eval '(%define (f n)
                       (%let ((a (%* n 2)))
                             (%if (%= n 0) 1 (%* n (f (%- n 1)))))))
(remote-eval '(%set! loops 0))
(remote-eval '(f (%* 10 30)))
(remote-eval 'loops)

(remote-eval '(%defmacro %gensym (%lambda (a) (%quote (%quote foo)))))

(remote-eval '(%define ((%gensym (%quote unique)) param1 param2) (%* param1 param2)))
(remote-eval '(%display (%gensym (%quote unique))))
(remote-eval '(foo 4 5))
;;; --------------------------------------------------------------------------
